...
class medoo
{
	...
	public function getColumns($table){
		...
		$get_columns = $this->query($sql)->fetchAll();
		return $get_columns;
	}
	public function getPKID($table){
		$getPKID = "";
		$sql = "SHOW COLUMNS FROM $table";
		$get_columns = $this->query($sql)->fetchAll();
		foreach ($get_columns as $primary_key) {
			if($primary_key['Key'] == "PRI" && $primary_key['Extra'] == "auto_increment"){
					$getPKID =  $primary_key['Field'];
			}
		}
		return $getPKID;
	}
